...
from transformers import set_seed, AutoTokenizer, get_linear_schedule_with_warmup, get_constant_schedule_with_warmup
import datasets
...
from efficient_tokenization.extend_embeddings import extend_model_embeddings, get_new_embeddings_grads, get_old_embeddings_grads, unfreeze_model, freeze_model_except_embeddings, freeze_old_embeddings, unfreeze_embeddings, unfreeze_first_last_layer
...
from efficient_tokenization.model_utils import forward_pass, move_optimizer_to_cpu, move_optimizer_to_gpu, calculate_grad_norm, save_checkpoint, remove_old_checkpoints, calc_batch_size_stuff, setup_lora
...
def get_next_batch(ds_iterator, ds_loader, epoch, epoch_step, skip_iterator=None):
    if skip_iterator is not None:
        try:
            ...
            batch = {k: v.cpu() for k, v in batch.items() if k != "task_type"}
...
def run_benchmark_loop(accelerator, model, config):
    ...
    # metrics = ["prompt_level_strict_acc,none", "inst_level_strict_acc,none", "prompt_level_loose_acc,none"]
    ...
    # tmp_tok.decode(samples[0]["input_ids"][0][0])
    # len(samples[0]["resps"][0][0])

    # len(samples[0]["input_ids"][0][0])
    # len(tmp_tok.encode(tmp_tok.decode(samples[0]["input_ids"][0][0])))
    # len(LM_model.tok_batch_encode(LM_model.tok_decode(samples[0]["input_ids"][0][0]))[0].tolist()[0])
    # self.tokenizer(
    #         strings,
    ...
    gathered_list = [metrics_dict, model_token_counts, theoretical_token_counts, old_theoretical_token_counts]
...
def run_evaluation_loop(model, eval_iterator, eval_loader, accelerator, num_steps: int, loss_types: List[str] = [], materialize_logits: bool = True):
    ...
    if True:
        with torch.no_grad():
            ...
            logger.debug(f"Device: {accelerator.process_index}, total steps: {num_steps}, steps in eval loader: {len(eval_loader)}", main_process_only=False)
            i = 0
            while i < num_steps:
                batch, eval_iterator, _, _, _ = get_next_batch(eval_iterator, eval_loader, epoch = -1, epoch_step = i, skip_iterator=None)
                mem_before = torch.cuda.memory_allocated() / 1024**2
                batch = {k: v.to(accelerator.device) for k, v in batch.items() if k != "task_type"}
...
def evaluate(model, eval_iterator, eval_loader, accelerator, num_steps=100, eval_config: Dict = None):
    model.eval()
    if eval_config is None:
        eval_config = {}
    results = {}
    
    # Run lm_eval only on main process
    if eval_config.get("run_lm_eval", False):
        lm_eval_results = run_benchmark_loop(accelerator, model, eval_config)
        # results["lm_eval"] = lm_eval_results
        results.update(lm_eval_results)
        # except Exception as e:
        #     logger.info(f"Error in lm_eval benchmark: {str(e)}", main_process_only=False)
        #     results["lm_eval"] = {"error": str(e)}

    # Ensure all processes synchronize after lm_eval
    accelerator.wait_for_everyone()
...
def main(args):
    ...
    # params_dict = convert_argparse_to_values(args)

    lr_schedule_prefreeze = args.lr_schedule  # this is not part of logging
    warmup_steps_prefreeze = args.warmup_steps
    if finetune_params_after_unfreeze != finetuning_params:
        lr_schedule_prefreeze = "constant"
        warmup_steps_prefreeze = unfreeze_params_steps / 10
        ...
        if checkpoint_path is not None:
            try:
                ...
                if resume_step > unfreeze_params_steps and unfreeze_params_steps > 0:
                    ...
                    current_freeze_params = finetune_params_after_unfreeze
    ...
    assert num_new_tokens == len(tokenizer) - model.config.vocab_size, f"tokenizer adding different number than num_new tokens: args: {args.num_new_tokens} != tok: {len(tokenizer) - model.config.vocab_size}"
    ...
    if task_name == "SFT":
        ...
        from efficient_tokenization.data_utils import MyPaddingCollatorGeneral
    ...
    # current_freeze_params = finetuning_params if not model_freezing_change_occured else finetune_params_after_unfreeze
    ...
    if state_dict is not None:
        ...
        try:
            ...
            logger.info(f"Resuming training at step {update_step} (epoch {epoch})")
    ...
    logger.info(f"learning rate {learning_rate}, warmup steps {warmup_steps} ({warmup_steps_prefreeze}), schedule_max_steps {max_train_steps}, lr_schedule {lr_schedule}, lr_schedule_prefreeze ({lr_schedule_prefreeze}), split_scheduler {split_scheduler}")
    logger.info(f"checkpointing steps {args.checkpointing_steps}")
    logger.info(f"eval steps: {args.eval_steps}, eval iters: {args.eval_iters}")
    ...
    while update_step < total_gradient_updates:
        if start_accumulating:
            ...
            if update_step % args.eval_steps == 0:
                ...
                torch.cuda.empty_cache()
                eval_metrics_dict = evaluate(model, eval_iterator, eval_loader, accelerator, args.eval_iters, eval_config)
                move_optimizer_to_gpu(optim)
                if accelerator.is_main_process:
                    # log_dict = {k: v for k, v in eval_metrics_dict.items() if not k.endswith("_weighted_loss")}
                    log_dict = {k: v for k, v in eval_metrics_dict.items() if k.endswith("_loss")}
            ...
            if finetune_params_after_unfreeze is not None and unfreeze_params_steps > 0 and update_step >= unfreeze_params_steps and not model_freezing_change_occured:
                logger.info(f"Changing model frozen parameters at step {update_step} from {finetuning_params} to {finetune_params_after_unfreeze}")
                accelerator.wait_for_everyone()
                ...
                unwrapped_model, model_is_frozen = manage_model_freezing(unwrapped_model, finetune_params_after_unfreeze, num_new_tokens, lora_configs)
        ...
        with accelerator.accumulate(model):  # Use accelerator's context manager
            ...
            if accelerator.sync_gradients:
                if num_new_tokens > 0:
                    ...
                    old_embeddings_list = get_old_embeddings_grads(model, num_new_tokens)
                    old_embeddings_grad_norm = calculate_grad_norm(old_embeddings_list, is_grad=True)
                    del new_embeddings_list, old_embeddings_list
